...
export class ContactUsComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.contactUsService
      ...
      .subscribe(
        (res: HttpResponse<IContactUs[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IContactUs): string {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInContactuses(): void {
    this.eventSubscriber = this.eventManager.subscribe('contactUsListModification', () => this.loadPage());
  }
